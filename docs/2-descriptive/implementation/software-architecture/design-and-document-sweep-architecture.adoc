...
The architecture requires a run audit record per attempt.
...
Rule: if `quantity <= threshold` then `is_low_stock = true`.

Also idempotent (only change when needed).
...
The sweep subsystem must produce evidence that it ran and what it did.

Minimum auditability standards:
