...
// NOTE (terminology inconsistency): This table uses 'Occupant' and 'Item' throughout. The project's ubiquitous
// language (terminology.adoc) defines the preferred terms as 'Stakeholder' (for any person with a role in a Home)
// and 'Stock' / 'Product' (for household items). 'Occupant' and 'Item' are not listed in the terminology table.
// These should be replaced with the canonical terms for consistency across all documentation.
// Additionally, 'Restock Planner' is referenced here but is not defined in terminology.adoc — add it there.
// Also missing from this table: events related to duplicate detection (Stock Duplicate detected,
// near-duplicate flagged, merge confirmed/rejected) and Membership/Invitation events (Invitation sent,
// accepted, declined, expired) which are in scope per DR-4, DR-5, and terminology.adoc.
...
|An Inventory has been generated.
|Triggered by the creation of an Item for the first time.
|The Inventory is created and reflects the presence of the new Item.
|Inventory, Item.

|A new Product has been generated. 
|Triggered by the addition of a new Item to the Inventory with no relation to an existing Product.
|The Inventory now reflects the presence of a new Product.
