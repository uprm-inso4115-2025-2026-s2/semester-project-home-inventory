...
== Implementation Flow (concise)

For quick reference, the canonical runtime flow is:

- Data Source (Supabase / network) → Repository (data → domain model mapping)
- Repository → Use Case (domain business rule invocation)
- Use Case → Cubit (presentation orchestration)
- Cubit → UI (via `BlocProvider` / `BlocBuilder` state emissions)

Keep conversions and DB logic inside `data/`, business rules in `domain/`, and UI-only logic in `presentation/`.
...
== 6. Risk Analysis Cross-Reference

Implementation-facing and documentation-facing Core Inventory risks are maintained in:

* `docs/3-analytic/concept-analysis/requirements_risk_analysis/core-inventory-risk-analysis.adoc`

Use this document together with the present state-management guide when evaluating architecture concerns, requirement uncertainty, and mitigation tracking.