...
== Introduction
This document presents the mockups for the Product Stock Screen developed in Figma to guide their implementation. The Product Stock screen allows users to view and manage specific purchased instances of a Product tracked in the inventory system. Each stock entry represents a distinct batch of a product, which may differ from other entries of the same product by attributes such as expiration date, quantity, status, owner, and notes.
...
The designed mockups follow the predefined product stock sequence diagram to ensure all possible views are covered. This document is found in `doc/2-descriptive/domain-description/events/product-stock/`
